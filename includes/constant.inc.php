<?php
    define("WEBSITE_NAME","Tribal & Farmer");
    define("ADMIN_PANAL","E-Marketplace ~ Vendor Panal");

    define("WEBSITE_PATH","http://localhost/pruthvi/collage_project/");
    define('SERVER_PATH',$_SERVER['DOCUMENT_ROOT'].'/pruthvi/collage_project/');

    define('SERVER_PRODUCT_IMAGE',SERVER_PATH."assets/products/");
    define('SITE_PRODUCT_IMAGE',WEBSITE_PATH."assets/products/");

    define('SERVER_BANNER_IMAGE',SERVER_PATH."assets/banner/");
    define('SITE_BANNER_IMAGE',WEBSITE_PATH."assets/banner/");

    define('SERVER_ABOUT_IMAGE',SERVER_PATH."assets/about/");
    define('SITE_ABOUT_IMAGE',WEBSITE_PATH."assets/about/");
?>