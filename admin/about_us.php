...
if(isset($_GET['type']) && $_GET['type']!=='' && isset($_GET['id']) && $_GET['id']>0){
	...
	if($type=='delete'){
		$res = mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM about_us WHERE id='$id'"));
		$oldImage = $res['image'];
		mysqli_query($con,"DELETE FROM about_us WHERE id='$id'");
		unlink(SERVER_ABOUT_IMAGE.$oldImage);
		redirect('about_us');
	}
...
$sql="select * from about_us order by order_number";
$res=mysqli_query($con,$sql);

?>
  <div class="card">
            <div class="card-body">
              <h1 class="grid_title">About Us Master</h1>
			  <a href="manage_about_us" class="add_link">Add About Us</a>
                  ...
                  <div class="table-responsive">
                    <table id="order-listing" class="table">
                      ...
                      <tbody>
						...
						<tr>
                            <td><?php echo $i?></td>
                            <td><img src="<?php echo SITE_ABOUT_IMAGE.$row['image']?>"/></td>
							...
							?></td>
							<td>
								<a href="manage_about_us?id=<?php echo $row['id']?>"><label class="badge badge-success hand_cursor">Edit</label></a>&nbsp;
								<?php
								if($row['status']==1){
								?>
