...
if(isset($_POST['submit'])){
	...
	if(mysqli_num_rows(mysqli_query($con,$sql))>0){
		...
		if($id==''){
			mysqli_query($con,"insert into category(category,order_no,status,added_on,added_by) values('$category','$order_number',1,'$added_on','$admin_name')");
		}else{
			mysqli_query($con,"update category set category='$category',order_no='$order_number',added_by='$admin_name' where id='$id'");
		}
